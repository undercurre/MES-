...
*│　接口名称： IAndonCallPersonRuleController                                      
*└──────────────────────────────────────────────────────────────┘
*/
...
namespace JZ.IMS.WebApi.Controllers  
{
	...
	[ApiController]
	public class AndonCallPersonRuleController : BaseController
	{
		private readonly IAndonCallPersonRuleRepository _repository;
		private readonly IMapper _mapper;
		private readonly IHttpContextAccessor _httpContextAccessor;
		private readonly IStringLocalizer<AndonCallPersonRuleController> _localizer;
		
		public AndonCallPersonRuleController(IAndonCallPersonRuleRepository repository, IMapper mapper, IHttpContextAccessor httpContextAccessor,
			IStringLocalizer<AndonCallPersonRuleController> localizer)
		{
        ...
        [HttpGet]
        [Authorize]
        public async Task<ApiBaseReturn<List<AndonCallPersonRuleAddOrModifyModel>>> LoadData([FromQuery]AndonCallPersonRuleRequestModel model)
        {
            ApiBaseReturn<List<AndonCallPersonRuleAddOrModifyModel>> returnVM = new ApiBaseReturn<List<AndonCallPersonRuleAddOrModifyModel>>();
            if (!ErrorInfo.Status)
            {
                try
                {
                    ...
                    if (!model.CALL_RULE_ID.IsNullOrWhiteSpace())
                    {
                        conditions += $"and (CALL_RULE_ID =:CALL_RULE_ID)";
                    }
                    ...
                    var viewList = new List<AndonCallPersonRuleAddOrModifyModel>();
                    list?.ForEach(x =>
                    {
                        var item = _mapper.Map<AndonCallPersonRuleAddOrModifyModel>(x);
        ...
        [HttpPost]
        [Authorize("Permission")]
        public async Task<ApiBaseReturn<bool>> SaveData([FromBody] AndonCallPersonRuleModel model)
        {
            ApiBaseReturn<bool> returnVM = new ApiBaseReturn<bool>();
            if (!ErrorInfo.Status)
            {
