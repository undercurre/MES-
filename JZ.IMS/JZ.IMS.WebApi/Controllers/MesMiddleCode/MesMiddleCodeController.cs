...
namespace JZ.IMS.WebApi.Controllers  
{
	...
	[ApiController]
	public class MesMiddleCodeController : BaseController
	{
		private readonly IMesMiddleCodeRepository _repository;
		...
		private readonly IStringLocalizer<MesMiddleCodeController> _localizer;
		
		public MesMiddleCodeController(IMesMiddleCodeRepository repository, 
			...
			IStringLocalizer<MesMiddleCodeController> localizer)
		{
        ...
        [HttpGet]
        [Authorize]
        public async Task<ApiBaseReturn<List<MesMiddleCodeListModel>>> LoadData([FromQuery]MesMiddleCodeRequestModel model)
        {
            ApiBaseReturn<List<MesMiddleCodeListModel>> returnVM = new ApiBaseReturn<List<MesMiddleCodeListModel>>();
            if (!ErrorInfo.Status)
            {
                try
                {
                    ...
                    var viewList = new List<MesMiddleCodeListModel>();
                    list?.ForEach(x =>
                    {
                        var item = _mapper.Map<MesMiddleCodeListModel>(x);
        ...
        [HttpPost]
        [Authorize("Permission")]
        public async Task<ApiBaseReturn<bool>> SaveData([FromBody] MesMiddleCodeModel model)
        {
            ApiBaseReturn<bool> returnVM = new ApiBaseReturn<bool>();
            if (!ErrorInfo.Status)
            {
        ...
        [HttpPost]
        [Authorize]
        public async Task<ApiBaseReturn<CollectMiddleCodeDataRequestModel>> SaveSite(SaveSiteModelcs siteModel)
        {
