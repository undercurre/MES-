...
*│　接口名称： ISmtFeederDefectController                                      
*└──────────────────────────────────────────────────────────────┘
*/
...
namespace JZ.IMS.WebApi.Controllers  
{
	...
	[ApiController]
	public class SmtFeederDefectController : BaseController
	{
		private readonly ISmtFeederDefectRepository _repository;
		private readonly IMapper _mapper;
		private readonly IHttpContextAccessor _httpContextAccessor;
		private readonly IStringLocalizer<ShareResourceController> _localizer;
		
		public SmtFeederDefectController(ISmtFeederDefectRepository repository, IMapper mapper, IHttpContextAccessor httpContextAccessor,
        ...
        [HttpGet]
        [Authorize]
        public async Task<ApiBaseReturn<List<SmtFeederDefectListModel>>> LoadData([FromQuery]SmtFeederDefectRequestModel model)
        {
            ApiBaseReturn<List<SmtFeederDefectListModel>> returnVM = new ApiBaseReturn<List<SmtFeederDefectListModel>>();
            if (!ErrorInfo.Status)
            {
                try
                {
                    ...
                    var viewList = new List<SmtFeederDefectListModel>();
                    list?.ForEach(x =>
                    {
                        var item = _mapper.Map<SmtFeederDefectListModel>(x);
        ...
        [HttpPost]
        [Authorize("Permission")]
        public async Task<ApiBaseReturn<bool>> SaveData([FromBody] SmtFeederDefectModel model)
        {
            ApiBaseReturn<bool> returnVM = new ApiBaseReturn<bool>();
            if (!ErrorInfo.Status)
            {
