...
*│　接口名称： IMesCommitVersionController                                      
*└──────────────────────────────────────────────────────────────┘
*/
...
namespace JZ.IMS.WebApi.Controllers  
{
	...
	[ApiController]
	public class MesCommitVersionController : BaseController
	{
		private readonly IMesCommitVersionRepository _repository;
		private readonly IMapper _mapper;
		private readonly IHttpContextAccessor _httpContextAccessor;
		private readonly IStringLocalizer<MesCommitVersionController> _localizer;
		
		public MesCommitVersionController(IMesCommitVersionRepository repository, IMapper mapper, IHttpContextAccessor httpContextAccessor,
			IStringLocalizer<MesCommitVersionController> localizer)
		{
        ...
        [HttpGet]
        [Authorize]
        public async Task<ApiBaseReturn<List<MesCommitVersionListModel>>> LoadData([FromQuery]MesCommitVersionRequestModel model)
        {
            ApiBaseReturn<List<MesCommitVersionListModel>> returnVM = new ApiBaseReturn<List<MesCommitVersionListModel>>();
            if (!ErrorInfo.Status)
            {
                try
                {
                    ...
                    var viewList = new List<MesCommitVersionListModel>();
                    list?.ForEach(x =>
                    {
                        var item = _mapper.Map<MesCommitVersionListModel>(x);
        ...
        [HttpPost]
        [Authorize]
        public async Task<ApiBaseReturn<bool>> SaveData([FromBody] MesCommitVersionModel model)
        {
            ApiBaseReturn<bool> returnVM = new ApiBaseReturn<bool>();
            if (!ErrorInfo.Status)
            {
