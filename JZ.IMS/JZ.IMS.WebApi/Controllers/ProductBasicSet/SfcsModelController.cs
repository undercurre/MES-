...
namespace JZ.IMS.WebApi.Controllers  
{
	...
	[ApiController]
	public class SfcsModelController : BaseController
	{
		private readonly ISfcsModelRepository _repository;
		private readonly IMapper _mapper;
		private readonly IHttpContextAccessor _httpContextAccessor;
		private readonly IStringLocalizer<SfcsModelController> _localizer;
		
		public SfcsModelController(ISfcsModelRepository repository, IMapper mapper, IHttpContextAccessor httpContextAccessor,
			IStringLocalizer<SfcsModelController> localizer)
		{
        ...
        [HttpGet]
        [Authorize]
        public async Task<ApiBaseReturn<List<SfcsModelListModel>>> LoadData([FromQuery]SfcsModelRequestModel model)
        {
            ...
            if (!ErrorInfo.Status)
            {
                try
                {
                    ...
                    {
                        conditions += $"and (instr(MODEL, :Key) > 0 or instr(DESCRIPTION, :Key) > 0)";
                    }
        ...
        [HttpPost]
        [Authorize("Permission")]
        public async Task<ApiBaseReturn<bool>> SaveData([FromBody] SfcsModelModel model)
        {
            ApiBaseReturn<bool> returnVM = new ApiBaseReturn<bool>();
            if (!ErrorInfo.Status)
            {
