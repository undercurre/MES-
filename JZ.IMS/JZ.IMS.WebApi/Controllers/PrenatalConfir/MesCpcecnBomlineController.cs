...
*│　作    者：嘉志科技                                              
*│　版    本：3.0   模板代码自动生成                                              
*│　创建时间：2020-06-30 15:36:44                           
*└──────────────────────────────────────────────────────────────┘
*┌──────────────────────────────────────────────────────────────┐
*│　命名空间： JZ.IMS.WebApi.Controllers                                   
*│　接口名称： IMesCpcecnBomlineController                                      
*└──────────────────────────────────────────────────────────────┘
*/
...
namespace JZ.IMS.WebApi.Controllers  
{
	...
	[ApiController]
	public class MesCpcecnBomlineController : BaseController
	{
		private readonly IMesCpcecnBomlineRepository _repository;
		private readonly IMapper _mapper;
		private readonly IHttpContextAccessor _httpContextAccessor;
		private readonly IStringLocalizer<MesCpcecnBomlineController> _localizer;
		
		public MesCpcecnBomlineController(IMesCpcecnBomlineRepository repository, IMapper mapper, IHttpContextAccessor httpContextAccessor,
			IStringLocalizer<MesCpcecnBomlineController> localizer)
		{
        ...
        [HttpGet]
        [Authorize]
        public async Task<ApiBaseReturn<List<MesCpcecnBomlineListModel>>> LoadData([FromQuery]MesCpcecnBomlineRequestModel model)
        {
            ApiBaseReturn<List<MesCpcecnBomlineListModel>> returnVM = new ApiBaseReturn<List<MesCpcecnBomlineListModel>>();
            if (!ErrorInfo.Status)
            {
                try
                {
                    ...
                    if (!model.SUPERCODE.IsNullOrWhiteSpace())
                    {
                        conditions += $"and SUPERCODE=:SUPERCODE ";
                    }
                    if (!model.BCITEMCODE.IsNullOrWhiteSpace())
                    {
                        conditions += $"and BCITEMCODE=:BCITEMCODE ";
                    }
                    var list = (await _repository.GetListPagedAsync(model.Page, model.Limit, conditions, "Id desc", model)).ToList();
                    var viewList = new List<MesCpcecnBomlineListModel>();
                    list?.ForEach(x =>
                    {
                        var item = _mapper.Map<MesCpcecnBomlineListModel>(x);
        ...
        [HttpPost]
        [Authorize("Permission")]
        public async Task<ApiBaseReturn<bool>> SaveData([FromBody] MesCpcecnBomlineModel model)
        {
            ApiBaseReturn<bool> returnVM = new ApiBaseReturn<bool>();
            if (!ErrorInfo.Status)
            {
