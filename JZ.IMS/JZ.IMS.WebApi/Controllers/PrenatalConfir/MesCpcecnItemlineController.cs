...
*│　作    者：嘉志科技                                              
*│　版    本：3.0   模板代码自动生成                                              
*│　创建时间：2020-06-30 15:37:59                           
*└──────────────────────────────────────────────────────────────┘
*┌──────────────────────────────────────────────────────────────┐
*│　命名空间： JZ.IMS.WebApi.Controllers                                   
*│　接口名称： IMesCpcecnItemlineController                                      
*└──────────────────────────────────────────────────────────────┘
*/
...
namespace JZ.IMS.WebApi.Controllers  
{
	...
	[ApiController]
	public class MesCpcecnItemlineController : BaseController
	{
		private readonly IMesCpcecnItemlineRepository _repository;
		private readonly IMapper _mapper;
		private readonly IHttpContextAccessor _httpContextAccessor;
		private readonly IStringLocalizer<MesCpcecnItemlineController> _localizer;
		
		public MesCpcecnItemlineController(IMesCpcecnItemlineRepository repository, IMapper mapper, IHttpContextAccessor httpContextAccessor,
			IStringLocalizer<MesCpcecnItemlineController> localizer)
		{
        ...
        [HttpGet]
        [Authorize]
        public async Task<ApiBaseReturn<List<MesCpcecnItemlineListModel>>> LoadData([FromQuery]MesCpcecnItemlineRequestModel model)
        {
            ApiBaseReturn<List<MesCpcecnItemlineListModel>> returnVM = new ApiBaseReturn<List<MesCpcecnItemlineListModel>>();
            if (!ErrorInfo.Status)
            {
                try
                {
                    ...
                    if (!model.ITEMCODE.IsNullOrWhiteSpace())
                    {
                        conditions += $" and ITEMCODE=:ITEMCODE ";
                    }
                    var list = (await _repository.GetListPagedAsync(model.Page, model.Limit, conditions, "Id desc", model)).ToList();
                    var viewList = new List<MesCpcecnItemlineListModel>();
                    list?.ForEach(x =>
                    {
                        var item = _mapper.Map<MesCpcecnItemlineListModel>(x);
        ...
        [HttpPost]
        [Authorize("Permission")]
        public async Task<ApiBaseReturn<bool>> SaveData([FromBody] MesCpcecnItemlineModel model)
        {
            ApiBaseReturn<bool> returnVM = new ApiBaseReturn<bool>();
            if (!ErrorInfo.Status)
            {
