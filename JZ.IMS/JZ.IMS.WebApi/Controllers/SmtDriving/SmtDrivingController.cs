...
namespace JZ.IMS.Admin.Controllers
{
    ...
    [ApiController]
    public class SmtDrivingController : BaseController
	{
        private readonly ISmtDrivingRecordMstRepository _repository;
        private readonly ISmtDrivingRecordDtlRepository _repository_dtl;
        private readonly IHttpContextAccessor _httpContextAccessor;
        private readonly ISfcsOperationLinesRepository _lineRepository;

        public SmtDrivingController(ISmtDrivingRecordMstRepository service, ISmtDrivingRecordDtlRepository repository_dtl, IHttpContextAccessor httpContextAccessor, ISfcsOperationLinesRepository lineRepository)
        {
            _repository = service;
		...
		{
			//待加入用户信息
			var LineList = _lineRepository.GetLinesList(ORGANIZE_ID, "SMT");
		...
		/// <returns></returns>	
		[HttpGet]
		public async Task<string> LoadData([FromQuery] SmtDrivingRecordMstRequestModel model)
		{
		...
		[HttpPost]
		[AllowAnonymous]
		public async Task<ApiBaseReturn<bool>> AddOrModifyAsync([FromBody] SmtDrivingRecordMst model)
		{
		...
		[HttpPost]
		[AllowAnonymous]
        public async Task<ApiBaseReturn<bool>> AddOrModifyAsync_Dtl([FromBody] SmtDrivingRecordDtl model)
        {
			ApiBaseReturn<bool> returnVM = new ApiBaseReturn<bool>();
			if (!ErrorInfo.Status)
			{
