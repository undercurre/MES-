...
namespace JZ.IMS.WebApi.Validation
{
    public class ManagerValidation : AbstractValidator<ManagerAddOrModifyModel>
    {
        public ManagerValidation()
        {
            ...
            RuleFor(x => x.USER_NAME).NotEmpty().Length(1, 32).WithMessage("用户账号不能为空并且长度不能超过32个字符");
            RuleFor(x => x.ROLE_ID).NotNull().WithMessage("用户所属角色不能为空") ;
            RuleFor(x => x.NICK_NAME).Length(1,32).WithMessage("用户昵称长度不能超过32个字符") ;
