...
namespace JZ.IMS.WebApi.Controllers
{
    ...
    [ApiController]
    public class SmtStationController : BaseController
    {
        private readonly ISmtStationRepository _repository;
        private readonly IMapper _mapper;
        private readonly IHttpContextAccessor _httpContextAccessor;
        private readonly IStringLocalizer<SmtStationController> _localizer;

        public SmtStationController(ISmtStationRepository repository, IMapper mapper, IHttpContextAccessor httpContextAccessor,
            IStringLocalizer<SmtStationController> localizer)
        {
        ...
        [HttpGet]
        [Authorize]
        public async Task<ApiBaseReturn<List<SmtStationListModel>>> LoadData([FromQuery]SmtStationRequestModel model)
        {
            ApiBaseReturn<List<SmtStationListModel>> returnVM = new ApiBaseReturn<List<SmtStationListModel>>();
            if (!ErrorInfo.Status)
            {
                try
                {
                    ...
                    {
                        conditions += $" AND INSTR(SMT_NAME, :SMT_NAME) > 0";
                    }
                    if (!model.TYPE.IsNullOrWhiteSpace())
                    {
                        conditions += $" AND INSTR(TYPE, :TYPE) > 0";
                    }
                    ...
                    var viewList = new List<SmtStationListModel>();
                    list?.ForEach(x =>
                    {
                        var item = _mapper.Map<SmtStationListModel>(x);
		...
		[HttpGet]
        [Authorize]
        public async Task<ApiBaseReturn<List<dynamic>>> ExportData([FromQuery]SmtStationRequestModel model)
        {
        ...
        [HttpPost]
        [Authorize("Permission")]
        public async Task<ApiBaseReturn<bool>> SaveData([FromBody] SmtStationModel model)
        {
