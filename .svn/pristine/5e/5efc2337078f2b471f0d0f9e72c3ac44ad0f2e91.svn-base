...
*│　接口名称： IMesHomeMemorandumController                                      
*└──────────────────────────────────────────────────────────────┘
*/
...
namespace JZ.IMS.WebApi.Controllers  
{
	...
	[ApiController]
	public class MesHomeMemorandumController : BaseController
	{
		private readonly IMesHomeMemorandumRepository _repository;
		private readonly IMapper _mapper;
		private readonly IHttpContextAccessor _httpContextAccessor;
		private readonly IStringLocalizer<MesHomeMemorandumController> _localizer;
		
		public MesHomeMemorandumController(IMesHomeMemorandumRepository repository, IMapper mapper, IHttpContextAccessor httpContextAccessor,
			IStringLocalizer<MesHomeMemorandumController> localizer)
		{
        ...
        [HttpGet]
        [Authorize]
        public async Task<ApiBaseReturn<List<MesHomeMemorandumListModel>>> LoadData([FromQuery]MesHomeMemorandumRequestModel model)
        {
            ApiBaseReturn<List<MesHomeMemorandumListModel>> returnVM = new ApiBaseReturn<List<MesHomeMemorandumListModel>>();
            if (!ErrorInfo.Status)
            {
                try
                {
                    ...
                    if (!model.TAR_DATE.IsNullOrWhiteSpace())
                    {
                        conditions += $"and ( instr(TAR_DATE, :TAR_DATE) > 0)";
                    }
                    if (!model.Key.IsNullOrWhiteSpace())
                    {
                        conditions += $"and (instr(TITLE, :Key) > 0 or instr(TAG, :Key) > 0)";
                    }
                    var list = (await _repository.GetListPagedAsync(model.Page, model.Limit, conditions, "Id desc", model)).ToList();
                    var viewList = new List<MesHomeMemorandumListModel>();
                    list?.ForEach(x =>
                    {
                        var item = _mapper.Map<MesHomeMemorandumListModel>(x);
        ...
        [HttpPost]
        [Authorize]
        public async Task<ApiBaseReturn<bool>> SaveData([FromBody] MesHomeMemorandumModel model)
        {
            ApiBaseReturn<bool> returnVM = new ApiBaseReturn<bool>();
            if (!ErrorInfo.Status)
            {
