...
*│　接口名称： ISfcsSiteMachineConfigController                                      
*└──────────────────────────────────────────────────────────────┘
*/
...
namespace JZ.IMS.WebApi.Controllers  
{
	...
	[ApiController]
	public class SfcsSiteMachineConfigController : BaseController
	{
		private readonly ISfcsSiteMachineConfigRepository _repository;
		private readonly IMapper _mapper;
		private readonly IHttpContextAccessor _httpContextAccessor;
		private readonly IStringLocalizer<SfcsSiteMachineConfigController> _localizer;
		
		public SfcsSiteMachineConfigController(ISfcsSiteMachineConfigRepository repository, IMapper mapper, IHttpContextAccessor httpContextAccessor,
			IStringLocalizer<SfcsSiteMachineConfigController> localizer)
		{
        ...
        [HttpGet]
        [Authorize]
        public async Task<ApiBaseReturn<List<SfcsSiteMachineConfigListModel>>> LoadData([FromQuery]SfcsSiteMachineConfigRequestModel model)
        {
            ApiBaseReturn<List<SfcsSiteMachineConfigListModel>> returnVM = new ApiBaseReturn<List<SfcsSiteMachineConfigListModel>>();
            if (!ErrorInfo.Status)
            {
                try
                {
                    ...
                    var viewList = new List<SfcsSiteMachineConfigListModel>();
                    list?.ForEach(x =>
                    {
                        var item = _mapper.Map<SfcsSiteMachineConfigListModel>(x);
        ...
        [HttpPost]
        [Authorize("Permission")]
        public async Task<ApiBaseReturn<bool>> SaveData([FromBody] SfcsSiteMachineConfigModel model)
        {
            ApiBaseReturn<bool> returnVM = new ApiBaseReturn<bool>();
            if (!ErrorInfo.Status)
            {
